...
class Series {
    ...
    static async createSeries({ title, description, release_year, rating, created_at, updated_at }) {
        const result = await pool.query(
    ...
    static async updateSeries(id, { title, description, release_year, rating, updated_at }) {
        const result = await pool.query(
